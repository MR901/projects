# build_index.py – convert docs/ into FAISS + pickle
...
for file in glob.glob("docs/**/*", recursive=True):
    ...
    # append tag glossary once per file
    if "tags" in info:
        tag_doc = "\n".join(f"* **{k}** – {v}" for k, v in info["tags"].items())
        chunks.append(f"HMI tag glossary for {info.get('id', info.get('model'))}\n" + tag_doc)
        meta.append(info)
...
print("✅ KB built:", len(chunks), "chunks")
