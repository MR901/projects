# app.py – Streamlit UI + Groq + RAG for vehicle mechanic & driving coach
import os, time
import streamlit as st
from dotenv import load_dotenv
from groq import Groq
import markdown as md
from system_prompt import VEHICLE_SYSTEM_PROMPT
from rag_support import fetch_context, guess_vehicle
...
# ── page css / banner ──────────────────────────────────────────────────────
st.set_page_config(page_title="🚗 Vehicle Mechanic & Driving Coach",
                   page_icon="🚗")
st.markdown("""
<style>
...
st.markdown(
    "<h1 style='text-align:center;margin:-1rem 0 .2rem;color:#0b3c91'>"
    "🚗 Vehicle Mechanic & Driving Coach</h1>",
    unsafe_allow_html=True,
)
...
if "messages" not in st.session_state:
    ...
    st.session_state.messages = [
        ...
        # ❷ Friendly greeting shown to the driver/rider
        {
            "role": "assistant",
            "content": (
                "Hi — I’m **Apex**, your digital mechanic and driving coach. "
                "Tell me about your car, bike, scooter or truck, and what it’s doing."
            ),
        },
    ]
...
def bubble(role, md_text, stream=False):
    ...
    st.markdown(
        f'<div class="bubble {css}">{html}{cursor}</div>',
        unsafe_allow_html=True,
    )
...
# ── user input ────────────────────────────────────────────────────────────
prompt = st.chat_input("Describe your vehicle and what you’re noticing …")
...
if prompt:
    ...
    for chunk in stream:
        ...
        tokens.append(delta)
        html = md.markdown(
            "".join(tokens), extensions=["nl2br", "tables", "fenced_code"]
        )
        placeholder.markdown(
            f'<div class="bubble assistant">{html}<span class="cursor"></span></div>',
            unsafe_allow_html=True,
        )
        time.sleep(0.03)
    ...
    final = "".join(tokens)
    placeholder.markdown(
        f'<div class="bubble assistant">{md.markdown(final, extensions=["nl2br","tables","fenced_code"])}</div>',
        unsafe_allow_html=True,
    )
...
if st.sidebar.button("📥 Download chat (.md)"):
    md_chat = []
    for m in st.session_state.messages[1:]:
        role = "**You:**" if m["role"] == "user" else "**Apex:**"
        md_chat.append(f"{role}\n\n{m['content']}\n")
    st.sidebar.download_button(
        "Save file",
        "\n---\n".join(md_chat),
        file_name="vehicle_chat.md",
        mime="text/markdown",
    )
